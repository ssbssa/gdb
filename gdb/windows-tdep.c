		   ...
		   enum info_proc_what what)
{
  ...
  std::optional<std::string> cmd_str, cwd_str, exe_str;
  CORE_ADDR tlb;
  gdb_byte buf[8];
...
static void
windows_core_info_proc (struct gdbarch *gdbarch, struct bfd *cbfd,
			const char *args, enum info_proc_what what)
{
  windows_info_proc (gdbarch, args, what);
}
...
void
entry_point_breakpoint::re_set (program_space *pspace)
{
...
windows_solib_ops::create_inferior_hook (int from_tty) const
{
  ...
  if (target_has_execution () && exec_base != 0)
      ...
      if (!entry_point_breakpoint_created)
	{
	  location_spec_up location
	    = new_address_location_spec (entry_point, nullptr, 0);
	  breakpoint *b = new entry_point_breakpoint (current_inferior ()->arch (),
						      std::move (location));
...
static LONGEST
windows_core_xfer_siginfo (struct gdbarch *gdbarch, struct bfd &cbfd,
			   gdb_byte *readbuf, ULONGEST offset, ULONGEST len)
{
  asection *section
    = bfd_get_section_by_name (&cbfd, ".coreexception");
  if (section == NULL)
    return -1;
...
#define EXC_SIZE_64 152
	...
	return -1;
      ...
      if (!bfd_get_section_contents (&cbfd, section, rec, 0, EXC_SIZE_64))
	return -1;
...
static const char *
windows_core_thread_name (struct gdbarch *gdbarch, bfd &cbfd,
			  struct thread_info *thr)
{
  ...
  sect = bfd_get_section_by_name (&cbfd, section_name.c_str ());
  ...
  if (bfd_get_section_contents (&cbfd, sect, buf, 0, bfd_section_size (sect)))
    {
      char *thread_name;
...
windows_init_abi_common (struct gdbarch_info info, struct gdbarch *gdbarch)
{
  ...
  set_gdbarch_core_info_proc (gdbarch, windows_core_info_proc);

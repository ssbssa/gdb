                        ...
                        for count, c in enumerate(childs, 1):
                            ...
                            if (count % 2) == 1:
                                ...
                                if isinstance(vc, basestring):
                                    ...
                                    vc_check = vc
                                    if vc_check.type.strip_typedefs().code in [
                                        gdb.TYPE_CODE_REF,
    ...
    def choices(self):
        ...
        if inferior and inferior.is_valid():
            ...
            if sel_thread and sel_thread.is_valid():
                ...
                try:
                    for thread in sorted(inferior.threads(), key=lambda t:t.num):
                        thread.switch()
...
has_tui = hasattr(gdb, "register_window_type")
        ...
        for c in self.commands:
            try:
                self.lines.extend(
                    [
                        output_convert(l)
                        for l in gdb.execute(c, to_string=True, styling=True).split("\n")
                    ]
                )

...
get_context_offset (Context *context, i386_gdbarch_tdep *tdep, int r)
{
  ...
  else if (I387_BND0R_REGNUM (tdep) > 0 && r >= I387_BND0R_REGNUM (tdep)
	   && r < I387_BNDCFGU_REGNUM (tdep))
    {
      context_offset = (char *) locate_xstate_feature
	(context, X86_XSTATE_BNDREGS_ID, NULL);
      context_offset += 16 * (r - I387_BND0R_REGNUM (tdep));
    }
  else if (I387_BNDCFGU_REGNUM (tdep) > 0 && r >= I387_BNDCFGU_REGNUM (tdep)
	   && r < I387_MPXEND_REGNUM (tdep))
    {
      context_offset = (char *) locate_xstate_feature
	(context, X86_XSTATE_BNDCFG_ID, NULL);
      context_offset += 8 * (r - I387_BNDCFGU_REGNUM (tdep));
    }
  else
...
windows_nat_target::info_proc (const char *args, enum info_proc_what what)
{
  ...
  ui_out_emit_table table_emitter (uiout, 3, -1, "files");
  uiout->table_header (8, ui_right, "handle", "Handle");
  uiout->table_header (16, ui_right, "type", "Type");
  uiout->table_header (50, ui_left, "path", "Path");
  uiout->table_body ();
			   ...
			   DUPLICATE_SAME_ACCESS))
	{
	  ...
	  if (object_type_name)
	      ...
	      if (!object_name)
		{
	      ...
	      uiout->field_unsigned ("handle", val);
	      uiout->field_string ("type", object_type_name.get ());
	      if (object_name)
		uiout->field_string ("path", object_name.get ());
	      else
		uiout->field_skip ("path");
	      uiout->text ("\n");
	    }
	}
...
windows_enable_btrace (ptid_t ptid, const struct btrace_config *conf)
{
  ...
  if (windows_process.ipt_threads == 0)
    {

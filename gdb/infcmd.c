...
get_callee_info (return_value_info *rv, frame_info_ptr frame)
{
  ...
  if (rv->function == nullptr)
    {
      CORE_ADDR real_pc = skip_language_trampoline (frame, pc);
      if (real_pc == 0)
	real_pc = gdbarch_skip_trampoline_code (get_frame_arch (frame),
						frame, pc);
      if (real_pc != 0)
	rv->function = find_pc_function (real_pc);
    }
  ...
  if (rv->function != nullptr)
	...
	= check_typedef (rv->function->type ()->target_type ());
      ...
      return_value = gdbarch_return_value_as_value

...
public:
					    ...
					    const struct btrace_config *conf) override
  {
    ...
    sprintf (sect_name, ".coreipt/%d", tp->ptid.lwp ());
    ...
    std::unique_ptr<btrace_target_info> tinfo
      { gdb::make_unique<btrace_target_info> (tp->ptid) };
				 ...
				 enum btrace_read_type type) override
  {
    ...
    sprintf (sect_name, ".coreipt/%d", btinfo->ptid.lwp ());
    asection *osec = bfd_get_section_by_name (core_bfd, sect_name);
    if (osec == nullptr)
      return BTRACE_ERR_UNKNOWN;
...
Restore a branch trace recording."),
	       &record_btrace_cmdlist);
  set_cmd_completer (record_btrace_restore_cmd, filename_completer);

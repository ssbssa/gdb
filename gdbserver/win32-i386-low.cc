...
get_context_offset (Context *context, int r, const target_desc *tdesc)
{
  ...
  int xmm16_regnum, ymm16h_regnum, zmm16h_regnum, bnd0r_regnum, bndcfg_regnum;
  ...
  else if ((xstate_features & X86_XSTATE_BNDREGS) != 0
	   && r >= (bnd0r_regnum = find_regno (tdesc, "bnd0raw"))
	   && r < bnd0r_regnum + num_mpx_bnd_registers)
    {
      context_offset = (char *) locate_xstate_feature
	(context, X86_XSTATE_BNDREGS_ID, NULL);
      context_offset += 16 * (r - bnd0r_regnum);
    }
  else if ((xstate_features & X86_XSTATE_BNDCFG) != 0
	   && r >= (bndcfg_regnum = find_regno (tdesc, "bndcfgu"))
	   && r < bndcfg_regnum + num_mpx_cfg_registers)
    {
      context_offset = (char *) locate_xstate_feature
	(context, X86_XSTATE_BNDCFG_ID, NULL);
      context_offset += 8 * (r - bndcfg_regnum);
    }
  else

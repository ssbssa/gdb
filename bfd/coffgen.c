	  ...
	  if (module.cvRecord.rva && module.cvRecord.size >= 24)
	    {
		  ...
		  || bfd_bread (sig, sizeof sig, abfd) != sizeof sig)
		goto fail;

	      if (sig[0] == 'R' && sig[1] == 'S'
		  && sig[2] == 'D' && sig[3] == 'S')
		{
		  sprintf (secname, ".corebuildid/%llx", module.base);
				     ...
				     bfd *exec_bfd)
{
  ...
  const char *exec_exe = exec_bfd->filename;
  if (!core_exe)
    return TRUE;

  last_delim = strrchr (core_exe, '/');
  if (last_delim)
    core_exe = last_delim + 1;
  last_delim = strrchr (core_exe, '\\');
  if (last_delim)
    core_exe = last_delim + 1;

  last_delim = strrchr (exec_exe, '/');
  if (last_delim)
    exec_exe = last_delim + 1;
  last_delim = strrchr (exec_exe, '\\');
  if (last_delim)
    exec_exe = last_delim + 1;

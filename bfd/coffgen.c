...
coff_core_file_p (bfd *abfd)
{
  ...
  if (threadNamesRva)
    {
	  ...
	  || bfd_read (&nameCount, sizeof nameCount, abfd) != sizeof nameCount)
	goto fail;

      for (n = 0; n < nameCount; n++)
	{
	  if (bfd_read (&name, sizeof name, abfd) != sizeof name)
	    goto fail;
  ...
  if (iptTraceRva)
      ...
      while (data_pos + sizeof ipt_header < ipt_data.traceSize)
	{
	  ...
	  sprintf (secname, ".coreipt/%u", ipt_header.threadId);
	  ...
	  if (rbo > 0)
	    {
	      sprintf (secname, ".coreipt/%u/2", ipt_header.threadId);
